...
import { Activity, BarChart3, Boxes, FileText, ListChecks, PlayCircle, RotateCcw, Search, ShieldAlert, TimerReset } from 'lucide-react';
import type { ReactNode } from 'react';

const navItems = [
  { label: 'Dashboard', href: '/accounting/event-engine/dashboard', icon: BarChart3 },
  { label: 'Event Explorer', href: '/accounting/event-engine/explorer', icon: Search },
  { label: 'Queue Monitor', href: '/accounting/event-engine/queue', icon: Boxes },
  { label: 'Validation', href: '/accounting/event-engine/validation', icon: ListChecks },
  { label: 'Retry Queue', href: '/accounting/event-engine/retries', icon: RotateCcw },
  { label: 'Dead Letter', href: '/accounting/event-engine/dead-letter', icon: ShieldAlert },
  { label: 'Monitoring', href: '/accounting/event-engine/monitoring', icon: Activity },
  { label: 'Reports', href: '/accounting/event-engine/reports', icon: FileText },
];

export function EventPageFrame({ title, description, children }: { title: string; description: string; children: ReactNode }) {
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 border-b border-border-default pb-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-text-muted">Accounting Event Engine</p>
        ...
        <Link
          href="/accounting/financial-calendar"
...
export function EventMetric({ label, value, note }: { label: string; value: string | number; note: string }) {
  return (
...
export function EventTable({ columns, children }: { columns: string[]; children: ReactNode }) {
  return (
...
export function EventBadge({ value }: { value?: string | null }) {
  const normalized = value ?? 'unknown';
  const tone =
    normalized === 'queued' || normalized === 'validated' || normalized === 'posted' || normalized === 'completed'
      ? 'bg-green-100 text-green-800'
      : normalized === 'failed' || normalized === 'dead_letter' || normalized === 'dead_letter_queue'
        ? 'bg-red-100 text-red-800'
      : normalized === 'retry_queue'
...
export function EventActionButton({ children, onClick, disabled }: { children: ReactNode; onClick: () => void; disabled?: boolean }) {
  return (
    <button
      type="button"
      onClick={onClick}
